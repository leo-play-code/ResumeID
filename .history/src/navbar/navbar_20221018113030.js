...
function Navbar(){
    const [navbar_state,setPageState] = useState('');
    ...
    useEffect(()=>{
        const init_url_arr = window.location.href.split('/')
        const init_url_page = init_url_arr[init_url_arr.length-1]
        console.log(init_url_page)
        setPageState(init_url_page)
    })
    return (
        <nav className="navbar">
            <h1>Huang Chien Hung (Leo)</h1>
            <span className='navbar-center'>
                {(navbar_state == '')&& <Link to="/" className='page-selected'>Home</Link>}
                {!(navbar_state == '')&&<Link to="/" onClick={()=>PageSelected('Home')}>Home</Link>}
